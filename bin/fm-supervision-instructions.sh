...
devin_checkpoint_seconds=${FM_DEVIN_WATCH_CHECKPOINT:-180}
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    devin)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision with a foreground checkpoint: bin/fm-watch-checkpoint.sh --seconds ' "$devin_checkpoint_seconds" '.'
      ;;
    pi|pi-signed)
