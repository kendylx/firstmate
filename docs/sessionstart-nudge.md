...
Codex's interactive TUI and the Devin REPL have no tracked session-open, compaction, or re-emit channel and are not covered by either tier.
...
| Devin | Uncovered | None. | Devin 3000.11.1 lists `session_start` hooks through `/hooks`, but hook delivery into model context is unverified, so Firstmate registers no tracked session-open adapter and does not claim instruction-refresh delivery for this surface; spawned devin workers receive the session-start instruction through their launch brief instead. |
